...
- **Summary**: Added configurable submit and newline shortcuts for the annotation modal.
- **Problem Solved**: Users had conflicting Enter/Shift+Enter habits, and the hardcoded behavior caused friction while editing annotations.
- **Feature Details**: Settings now let users choose `None`, `Enter`, `Shift+Enter`, or `Ctrl+Enter` for submit and newline; modal hint text stays in sync with the current configuration.
- **Technical Implementation**:
  - Added shortcut normalization and key-label helpers in `modal-shortcuts.ts`.
  - Updated `main.ts` settings, modal wiring, and localized strings for synchronized shortcut hints.

### Feature 2: Mobile Editing Layout Refinements
- **Summary**: Refined the annotation modal layout for mobile editing, especially on iOS.
- **Problem Solved**: The previous modal wasted vertical space, making the textarea too small when the on-screen keyboard was open.
- **Feature Details**: The modal is wider on mobile, the title stays on one line, color swatches stay on one row, the cancel button was removed, and the primary confirm action remains accessible beside the close icon.
- **Technical Implementation**:
  - Updated mobile modal styles in `styles.css` to widen the dialog and keep the header, color picker, and action area compact.
  - Simplified modal actions in `main.ts` so only the close icon and confirm button remain.

### Feature 3: Automatic Normalization for Unsafe Annotation Newlines
- **Summary**: Added automatic normalization for raw newlines inside `data-note` attributes.
- **Problem Solved**: Direct source edits that inserted raw line breaks inside `data-note` could leave notes visible temporarily but fail when the file was reopened in Obsidian.
- **Feature Details**: The plugin now converts unsafe raw newlines to `&#10;` after editor idle time and on save, with a settings toggle to control the behavior and the manual repair command reusing the same normalization logic.
- **Technical Implementation**:
  - Added shared normalization helpers in `annotation-normalization.ts`.
  - Wired idle-time editor normalization and save-time vault fallback in `main.ts`, preserving cursor position when rewriting note content.
