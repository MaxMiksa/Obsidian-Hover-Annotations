## v2.1.0 – 光标感知的批注规范化 (2026年03月20日)

### 功能 1：光标感知的自动规范化时机
- **总结**：优化了批注换行自动规范化的触发时机，只有在光标离开批注源码后才会执行。
- **解决痛点**：旧版“编辑后空闲一段时间就改写”的逻辑，可能在用户还在编辑 span 源码时就重写 `data-note`，导致源码突然折叠，打断输入。
- **功能细节**：现在只有当光标移出批注源码并在外部短暂停留后，插件才会把不安全的原始换行转换为合法格式，因此在 `data-note` 内连续编辑时不会再被中途打断。
- **技术实现**：
  - 将主触发链从 `main.ts` 里的 workspace 级空闲检测，迁移到 live preview `ViewPlugin` 的 selection/update 流程。
  - 在 `annotation-normalization.ts` 中新增光标离开后的调度判断逻辑，用于决定何时 schedule、cancel 或保持不动作。

### 功能 2：规范化设置进一步简化
- **总结**：将自动规范化设置收敛为单一开关，仅保留“移出批注源码后自动规范化”。
- **解决痛点**：在根因已经明确为“编辑源码时过早改写”的情况下，同时保留两个自动修复入口，会让行为边界更难理解。
- **功能细节**：设置页现在只保留“移出批注源码后自动规范化”；保存时保底检查已移除，手动修复能力仍然保留。
- **技术实现**：
  - 从 `main.ts` 中移除了基于 vault `modify` 的保存兜底逻辑以及对应设置接线。
  - 在 `tests/annotation-normalization.test.ts` 中补充了对 schedule、cancel 和光标离开后规范化规则的回归测试。

## v2.0.0 – 移动端弹窗与安全换行控制 (2026年03月19日)

### 功能 1：批注弹窗快捷键可配置
- **总结**：为批注弹窗新增“完成批注”和“换行”快捷键设置。
- **解决痛点**：不同用户对 Enter / Shift+Enter 的使用习惯不一致，固定按键行为会影响输入体验。
- **功能细节**：设置页现在支持为“完成批注”和“换行”分别选择 `无`、`Enter`、`Shift+Enter` 或 `Ctrl+Enter`；弹窗顶部提示会与当前配置实时保持一致。
- **技术实现**：
  - 新增 `modal-shortcuts.ts`，统一处理快捷键标签、事件匹配与冲突规范化。
  - 在 `main.ts` 中补齐设置项、弹窗接线和中英文文案同步逻辑。

### 功能 2：移动端编辑弹窗布局优化
- **总结**：针对手机端，尤其是 iOS 键盘弹出场景，优化了批注弹窗布局。
- **解决痛点**：旧版弹窗占用了过多垂直空间，导致输入框在手机上过小，几乎无法正常编辑。
- **功能细节**：移动端弹窗更宽，标题保持单行，颜色圆点压缩为单排，取消按钮被移除，仅保留右上角关闭和主确认按钮。
- **技术实现**：
  - 在 `styles.css` 中重写移动端弹窗宽度、标题、颜色区和操作区布局。
  - 在 `main.ts` 中简化弹窗操作区，移除冗余取消按钮。

### 功能 3：不安全批注换行自动规范化
- **总结**：新增对 `data-note` 中原始换行的自动规范化能力。
- **解决痛点**：用户直接在源码里编辑批注并插入原始回车时，文件当下可能还能显示，但重新打开时会触发 Obsidian 打开失败。
- **功能细节**：插件现在会在编辑器空闲后和文件保存后，将 `data-note` 中不安全的原始换行自动转换成 `&#10;`；设置页可开关此行为，手动修复命令也复用同一套逻辑。
- **技术实现**：
  - 新增 `annotation-normalization.ts` 作为共享规范化模块。
  - 在 `main.ts` 中接入编辑器空闲修复、保存兜底修复和光标位置保持逻辑。
